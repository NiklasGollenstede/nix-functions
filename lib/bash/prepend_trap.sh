...
#  NOTE: When calling this in a sub-shell whose parent may already have traps installed, make sure to call »trap - trapName« first. On a new shell, this should be a no-op, but without it, the parent shell's traps will be added to the sub-shell as well (due to strange behavior of »trap -p« (in bash ~5.1.8)).
